...
export const archiveCategory = async (req, res) => {
  try {
    ...
    const categoryArchive = await Category.findById(categoryId);
    if (!categoryArchive) {
...
export const copyCategory = async (req, res) => {
  try {
    const { categoryId } = req.params;
    const categoryToCopy = await Category.findById(categoryId);
...
export const getArchivedCategories = async (req, res) => {
  try {
    const categories = await Category.find({ archived: true });
